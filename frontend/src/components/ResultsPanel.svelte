<script lang="ts">
  ...
  const attempt = $derived(sounding(playedEvents(played, result.feedback)));
  /** The same bars as notated, at the tempo that was counted in. */
  const written = $derived(sounding(writtenEvents(exercise.expected_notes, exercise.tempo_bpm)));
