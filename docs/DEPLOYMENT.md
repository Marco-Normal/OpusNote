...
| `MidiAllowedForUrls` policy | grants the MIDI permission with no prompt on builds that gate Web MIDI behind one — nobody is sitting there to click Allow. Current Chromium defines no MIDI policy at all, so there the line is inert rather than harmful |
| `AudioCaptureAllowedUrls` + `AudioCaptureAllowed: false` | the first auto-grants the microphone to the notebook's own origin. The second is what makes that a list rather than a question: left unset, Chromium prompts for every *other* origin, and on a kiosk nobody is there to answer; set false, the listed URLs keep their grant and every other origin is refused without a prompt. The first name has no `ForUrls` form: spelling it `AudioCaptureAllowedForUrls` by analogy with the MIDI policy above leaves the list empty, and with prompts off that refuses the microphone to every origin including the notebook's own |
...
| The sampled piano plays as the synthesiser | The device bar says *samples failed to load* when the sample set cannot be decoded — the files are there but unreadable. Re-install with `curl -s -X POST localhost:8000/api/audio/piano`; if it persists, the status payload names the directory to check. |
| No sound at all, and the audio pill says `suspended` | The browser may only start audio after a click. Click anywhere in the page — the pill should turn to `audio ok` — or press *Test* next to the instrument choice. |
