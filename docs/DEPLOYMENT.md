...
| `AudioCaptureAllowedForUrls` + `AudioCaptureAllowed: false` | the first auto-grants the microphone to the notebook's own origin. The second is what makes that a list rather than a question: left unset, Chromium prompts for every *other* origin, and on a kiosk nobody is there to answer; set false, the listed URLs keep their grant and every other origin is refused without a prompt |
